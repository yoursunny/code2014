...
PhoneParser.prototype={
	initialize:function(n) {
		...
		else if ((m=/^(0\d+)-(\d{3,})$/.exec(n))!=null)//021-54740000形式
		{
			this.area=m[1];
			this.num=m[2];
		}
		...
		else if (/^\d{6,}$/.test(n))//00862154740000,02154740000,54740000,008613800138000,13800138000,013800138000形式
		{
			if (n.startsWith('00'))
			{
			...
			if (n.startsWith('0'))
			{
				if (n.startsWith('01')) {
					if (n.startsWith('010')) {
						this.area='010';
						this.num=n.substr(6);
					} else if (/^0(13\d|15[078936]|189)\d{8}$/.test(n)) {
						this.area='';
						this.num=n.substr(1);
		...
		if (/^(13\d|15[078936]|189)\d{8}$/.test(this.num))//手机
		{
			this.area='';
			this.type=PhoneParser.Mobile;
			this.local=PhoneParser.NeedQuery;
			if (/^(13[56789]\d|15[089]\d|134[^9]\d)\d{7}$/.test(this.num))
			{
				this.company=PhoneParser.CMCC;
				this.network=PhoneParser.GSM;
			}
			else if (/^157\d{8}/.test(this.num)) {
				this.company=PhoneParser.CMCC;
				this.network=PhoneParser.TDSCDMA;
			}
			else if (/^(13[012]|156)\d{8}$/.test(this.num))
			{
				this.company=PhoneParser.Unicom;
				this.network=PhoneParser.GSM;
			}
			else if (/^(133|153|189)\d{8}$/.test(this.num))
			{
				this.company=PhoneParser.Telecom;
				this.network=PhoneParser.CDMA;
			}
